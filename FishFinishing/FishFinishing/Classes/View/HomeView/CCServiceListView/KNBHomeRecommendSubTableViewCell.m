...
- (void)setModel:(KNBHomeServiceModel *)model {
    ...
    //计算名称最大长度
    self.maxSpace.constant = KNB_SCREEN_WIDTH - 168;
    ...
    if (isNullArray(model.caseList)) {
        self.leftCase.image = KNBImages(@"knb_default_case");
        self.middleCase.image = KNBImages(@"knb_default_case");
        self.rightCase.image = KNBImages(@"knb_default_case");
    } else {
